  ...
  return (
    <section className="space-y-6">
      <header>
        ...
        <p className="text-sm text-(--color-muted)">
          Modifica i prompt che il bot invia a GPT. Le modifiche vengono usate dal prossimo job
          schedulato (nessun riavvio richiesto).
        </p>
      </header>
              ...
              <button
                ...
                className={`block w-full rounded-lg px-3 py-2 text-left text-sm transition-colors ${
                  activeKey === key
                    ...
                    : "text-(--color-muted) hover:bg-slate-800/60"
                }`}
              >
                {PROMPT_LABELS[key]}
              </button>
    ...
    <div>
      ...
      <div className="max-h-[60vh] overflow-y-auto space-y-3">
        ...
        {versions.data?.versions.map((v) => (
          <div
            key={v.id}
            className="rounded-lg border border-(--color-line) bg-slate-950/50 p-3"
          >
            ...
            {v.comment && (
              <p className="mt-2 rounded-md bg-slate-900/60 px-3 py-1.5 text-xs italic text-(--color-muted)">
                {v.comment}
              </p>
            )}
            <pre className="mt-2 max-h-40 overflow-auto rounded-md bg-slate-950 p-2 text-xs leading-relaxed text-(--color-text)">
              {v.content}
            </pre>
          </div>
        ))}
