...
export function TradeRow({ trade: t, onEdit, onClose }: TradeRowProps) {
  ...
  return (
    <tr className="bg-(--color-panel)/40 transition-colors hover:bg-(--color-hover)/60 [&>td]:border-y [&>td]:border-(--color-line)">
      <td className="sticky left-0 z-20 w-14 min-w-14 bg-(--color-panel) px-3 py-2 text-center">
        <Button
          size="icon"
      ...
      <td className="sticky left-14 z-20 w-14 min-w-14 bg-(--color-panel) px-3 py-2 text-center">
        <Button
          size="icon"
          variant="secondary"
          className="size-8"
      ...
      <td className="sticky left-28 z-20 w-40 min-w-40 whitespace-nowrap bg-(--color-panel) px-3 py-2 font-medium shadow-[1px_0_0_var(--color-line)]">
        <Link
      ...
      <td className={`tnum whitespace-nowrap px-3 py-2 text-right ${pnlClass(pnl)}`}>
        {formatCurrency(pnl, t.account_currency || "EUR")}
      </td>
      <td
        className={`tnum whitespace-nowrap px-3 py-2 text-right ${
          pnlPct == null ? "text-(--color-muted)" : pnlClass(pnlPct)
        }`}
      >
      ...
      <td className="whitespace-nowrap px-3 py-2 text-(--color-muted)">{t.category}</td>
      <td className="whitespace-nowrap px-3 py-2 text-(--color-muted)">{t.direction}</td>
      <td className="tnum whitespace-nowrap px-3 py-2 text-right">
        {formatNumber(t.target_entry_price)}
      </td>
      <td className="tnum whitespace-nowrap px-3 py-2 text-right">
        {formatNumber(t.take_profit)}
      </td>
      <td className="tnum whitespace-nowrap px-3 py-2 text-right">
      ...
      <td className="tnum whitespace-nowrap px-3 py-2 text-right">
        {formatNumber(t.high_water_mark)}
      </td>
      <td className="tnum whitespace-nowrap px-3 py-2 text-right">
        {formatNumber(t.trailing_stop_distance)}
      </td>
      <td
        className={`tnum whitespace-nowrap px-3 py-2 text-right ${
